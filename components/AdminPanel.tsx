      ...
      <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="grid gap-6 bg-slate-900 p-8 rounded-3xl border border-slate-800">
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-500 uppercase px-1">Título do Quiz</label>
              <input 
                type="text" 
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-500 uppercase px-1">Descrição</label>
              <textarea 
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="font-bold text-slate-400 uppercase tracking-widest text-xs px-1">Perguntas ({editingQuiz.questions?.length || 0})</h3>
          ...
          {editingQuiz.questions?.map((q, qIdx) => (
            <div key={q.id} className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl relative group">
              <button 
                onClick={() => {
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {q.options.map((opt, oIdx) => (
                  <div key={opt.id} className="flex gap-2">
                    <input 
                      type="text" 
                      className={`flex-1 bg-slate-950 border p-3 rounded-xl text-sm outline-none transition-all ${opt.isCorrect ? 'border-emerald-500/50 ring-1 ring-emerald-500/20' : 'border-slate-800'}`}
                    ...
                    <button
                      ...
                      className={`px-4 rounded-xl border transition-all ${opt.isCorrect ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-500'}`}
        ...
        <div className="flex gap-4 sticky bottom-8 bg-slate-950/80 backdrop-blur-xl p-4 rounded-3xl border border-slate-900 shadow-2xl">
          ...
          <Button variant="primary" className="flex-1" onClick={handleSave} isLoading={isSaving}>SALVAR QUIZ</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <section className="bg-indigo-600 p-10 rounded-3xl text-white relative overflow-hidden shadow-2xl shadow-indigo-500/20">
        <div className="relative z-10 max-w-lg">
          ...
          <p className="text-indigo-100 mb-8 opacity-90 leading-relaxed font-medium">
            Use nossa inteligência artificial para criar um quiz completo em segundos. Basta digitar o tema!
          </p>
      ...
      <section>
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-bold text-slate-400 uppercase tracking-widest text-xs px-1">Gerenciar Quizzes</h3>
        ...
        <div className="grid gap-4">
          {quizzes.map(q => (
            <div key={q.id} className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex items-center justify-between group hover:border-indigo-500/30 transition-all">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{q.category} &bull; Armazenado em Banco de Dados</p>
                </div>
              </div>
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={() => handleEdit(q)} title="Editar">
                  <i className="fas fa-edit"></i>
                </Button>
                <Button variant="ghost" size="sm" className="hover:text-rose-500" onClick={() => handleDelete(q.id)} title="Excluir">
                  <i className="fas fa-trash"></i>
                </Button>
              </div>
            </div>
          ...
          {quizzes.length === 0 && (
            <div className="text-center py-10 border-2 border-dashed border-slate-800 rounded-3xl">
              <p className="text-slate-600 font-bold uppercase tracking-widest text-xs">Nenhum quiz encontrado</p>
            </div>
          )}
        </div>
      </section>
